#!/usr/bin/env python3
"""
Aplicación WSGI para LasaCam - Compatible con cPanel/Passenger
...
def handle_upload(environ):
    ...
    try:
        ...
        protocol = 'https' if environ.get('HTTPS') == 'on' or environ.get('HTTP_X_FORWARDED_PROTO') == 'https' else 'http'
        
        # Construir la URL base considerando si está en subdirectorio
        script_name = environ.get('SCRIPT_NAME', '')
        base_path = script_name.rstrip('/')
        file_url = f"{protocol}://{host}{base_path}/uploads/{unique_filename}"
            ...
            for file_path in files:
                if file_path.is_file() and file_path.suffix.lower() in ALLOWED_EXTENSIONS:
                    photos.append({
                        ...
                        'url': f"{protocol}://{host}{base_path}/uploads/{file_path.name}"
                    })
...
def serve_upload_file(environ, path):
    ...
    try:
        # Extraer nombre del archivo (remover cualquier prefijo de ruta)
...
def application(environ, start_response):
    ...
    if '/uploads/' in path:
        ...
        if result[0] is None:
            status = '404 Not Found' if result[1] == 404 else '500 Internal Server Error'
    ...
    # Manejar /api/upload
    if path.endswith('/api/upload') and method == 'POST':
    ...
    # Manejar /api/photos
    if path.endswith('/api/photos') and method == 'GET':
    ...
    headers = [('Content-Type', 'application/json')] + cors_headers
    start_response(status, headers)
    return [json.dumps({'error': 'Ruta no encontrada', 'path': path, 'method': method}).encode('utf-8')]
